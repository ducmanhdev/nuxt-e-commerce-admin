...
const ORIGIN_COLUMNS = [
  ...
  { key: 'billboardId', label: 'Billboard', sortable: true },
...
const { data: billboards } = useReferenceBillboard()
</script>
...
<template>
  <UCard>
    ...
    >
      <template #billboardId-data="{ row }">
        {{ billboards?.find((item) => item.id === row.billboardId)?.name || 'N/A' }}
      </template>
